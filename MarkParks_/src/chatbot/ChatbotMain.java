...
public class ChatbotMain {
	...
	* @param keyword
	* @param startpsn
	* @return the index of the keyword after startPsn that is isolated and has no negations of -1 otherwise
	*/
	public static int findKeyword(String searchString, String keyword, int startPsn) {
		...
		while(psn >= 0) {
			if(keywordIsIsolated(psn,keyword,searchString) && noNegations(searchString, psn)) {
				...
				psn = searchString.indexOf(keyword, psn+1);//returns the index of next keyword
			}
		}
		return -1;
	}
