...
public class Searching {
	...
	private int binarySearch(int[] searchThis, int start, int end, int target) {
		int middle = (start + end)/2;
		if(searchThis[middle] == target)
			return middle;
		else
			if(searchThis[middle] > target)
				return binarySearch(searchThis, start, middle+1, target);
			else return binarySearch(searchThis, middle-1, end, target);
	}
